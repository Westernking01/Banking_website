...
// Static fallback chart for UI purposes
const chartBars = [
  ...
  return (
    <div className="p-8 space-y-8 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-12 gap-6">
        {/* Balance Card */}
        <div className="col-span-12 lg:col-span-8 relative overflow-hidden rounded-3xl bg-primary shadow-2xl p-8 text-white">
          ...
          <div className="relative z-10 flex flex-col justify-between h-full">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0">
              <div>
                <p className="text-blue-200 text-xs sm:text-sm font-medium mb-1 tracking-wide">TOTAL BALANCE</p>
                <h1 className="text-4xl sm:text-5xl font-black font-headline tracking-tight">${balance}</h1>
              </div>
            ...
            <div className="mt-8 sm:mt-12 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 sm:gap-0">
              <div className="flex gap-6 sm:gap-8">
                <div>
                  <p className="text-blue-200 text-[10px] mb-1 font-bold">MONTHLY INCOME</p>
                  <p className="text-base sm:text-lg font-bold font-headline">${stats.monthlyIncome.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
                <div>
                  <p className="text-blue-200 text-[10px] mb-1 font-bold">MONTHLY EXPENSES</p>
                  <p className="text-base sm:text-lg font-bold font-headline">${stats.monthlyExpenses.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
              </div>
        ...
        <div className="col-span-12 lg:col-span-4 grid grid-cols-1 gap-4">
          <Link to="/transfer" className="group relative flex items-center justify-between p-6 bg-secondary-container rounded-2xl hover:bg-secondary-fixed transition-colors">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center shadow-sm">
                <span className="material-symbols-outlined text-secondary font-bold">send</span>
              </div>
              <span className="font-bold text-on-secondary-container font-headline">Send Money</span>
            </div>
            <span className="material-symbols-outlined text-on-secondary-container opacity-0 group-hover:opacity-100 transition-opacity">arrow_forward</span>
          </Link>
          <button onClick={() => setIsReceiveModalOpen(true)} className="group relative flex items-center justify-between p-6 bg-white rounded-2xl border border-transparent hover:border-secondary transition-all">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-secondary/10 rounded-xl flex items-center justify-center">
                <span className="material-symbols-outlined text-secondary font-bold">call_received</span>
              </div>
              <span className="font-bold text-primary font-headline">Receive</span>
            </div>
          ...
          <button onClick={() => setIsPayBillModalOpen(true)} className="group relative flex items-center justify-between p-6 bg-white rounded-2xl border border-transparent hover:border-secondary transition-all">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-secondary/10 rounded-xl flex items-center justify-center">
                <span className="material-symbols-outlined text-secondary font-bold">payments</span>
              </div>
              <span className="font-bold text-primary font-headline">Pay Bills</span>
            </div>
            <span className="material-symbols-outlined text-primary opacity-0 group-hover:opacity-100 transition-opacity">arrow_forward</span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 xl:col-span-7 space-y-6">
          ...
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
              <h2 className="text-xl font-bold font-headline text-primary">Spending Analytics</h2>
              <div className="flex bg-surface-container-low p-1 rounded-lg self-stretch sm:self-auto">
                <button 
                  onClick={() => setTimeframe('hourly')}
                  className={`flex-1 sm:flex-none px-4 py-1 text-xs font-bold rounded-md transition-colors ${timeframe === 'hourly' ? 'bg-white shadow-sm text-primary' : 'text-slate-500 hover:text-primary'}`}
                >
                  Hourly
                </button>
                <button 
                  onClick={() => setTimeframe('daily')}
                  className={`flex-1 sm:flex-none px-4 py-1 text-xs font-bold rounded-md transition-colors ${timeframe === 'daily' ? 'bg-white shadow-sm text-primary' : 'text-slate-500 hover:text-primary'}`}
                >
                  Daily
                </button>
                <button 
                  onClick={() => setTimeframe('monthly')}
                  className={`flex-1 sm:flex-none px-4 py-1 text-xs font-bold rounded-md transition-colors ${timeframe === 'monthly' ? 'bg-white shadow-sm text-primary' : 'text-slate-500 hover:text-primary'}`}
                >
                  Monthly
                </button>
              </div>
                ...
                <div key={idx} className="flex flex-col items-center gap-2 flex-1">
                  ...
                  >
                    {active && label && label !== '$0' && (
                      <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-primary text-white text-[10px] font-bold py-1 px-2 rounded">{label}</div>
                    )}
                    {!active && (
                      <div className="absolute inset-0 bg-primary/20 rounded-t-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                         {label !== '$0' && <span className="text-[10px] text-white font-bold opacity-0 group-hover:opacity-100 transition-opacity absolute -top-6">{label}</span>}
                      </div>
                    )}
                  </div>
                  <span className={`text-[10px] font-bold ${active ? 'text-primary' : 'text-slate-400'}`}>{day}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold font-headline text-primary">Recent Transactions</h2>
        ...
        <div className="col-span-12 xl:col-span-5 space-y-6">
          ...
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-bold font-headline text-primary">My Cards</h2>
              <Link to="/cards" className="p-2 bg-slate-100 rounded-lg">
                <span className="material-symbols-outlined text-sm">add</span>
              </Link>
            </div>
            {/* Credit Card Visual */}
            <div className="aspect-[1.6/1] w-full rounded-2xl bg-gradient-to-br from-slate-900 to-slate-700 p-6 relative overflow-hidden text-white flex flex-col justify-between shadow-lg">
              ...
              <div className="relative z-10 flex justify-between items-start">
                ...
                <span className="font-bold tracking-widest text-xs">VAULT PLATINUM</span>
              </div>
              <div className="relative z-10 space-y-4">
                <p className="text-xl font-headline tracking-[0.2em] font-medium">•••• •••• •••• 8842</p>
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-[10px] opacity-60 mb-0.5">CARD HOLDER</p>
                    <p className="text-sm font-bold">{user?.name?.toUpperCase() || 'VAULT MEMBER'}</p>
                  </div>
          ...
          {/* Upcoming Bills */}
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            <h2 className="text-lg font-bold font-headline text-primary mb-6">Upcoming Bills</h2>
            <div className="space-y-4">
              {bills.length > 0 ? bills.map((bill, i) => (
                <div key={i} className="flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 ${bill.bg} ${bill.color} rounded-xl flex items-center justify-center`}>
            ...
            <button className="w-full mt-6 py-3 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 text-sm font-bold hover:border-primary hover:text-primary transition-all">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-primary/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl relative">
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-on-surface-variant">Select a bill from your upcoming schedule to pay securely from your balance.</p>
            </div>
            
            <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
              {bills.length > 0 ? bills.map((bill, i) => (
                <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl border border-outline-variant/30 hover:border-secondary/50 transition-colors gap-4">
                  <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center justify-between sm:flex-col sm:items-end gap-2">
                    ...
                    <button 
                      onClick={() => handlePayBill(bill)}
                      disabled={isPaying}
                      className="px-4 py-2 bg-primary text-white rounded-xl text-[10px] uppercase font-bold tracking-widest hover:opacity-90 disabled:opacity-50 transition-opacity"
                    >
                      {isPaying ? 'Paying...' : 'Pay'}
                    </button>
                  </div>
